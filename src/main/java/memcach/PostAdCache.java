    ...
    public PostAdCache(){
        ...
        for (UUID id: idAll) {
            ...
            if (account.getEmail().equals("111")){
                ...
                postAd = new PostAd(account.getId(), "ЧИП", textCheep, listCheep.get(2), listCheep.get(3), listCheep);
                postAd2 = new PostAd(account.getId(), "Ампер -вихрь позитивной энергии и неиссякаемой любви!", textAmper, listAmper.get(0), listAmper.get(1), listAmper);
                postAd3 = new PostAd(account.getId(), "Маленькая трусишка. Живет в приюте и мечтает уехать домой!", textBo, listBo.get(0), listBo.get(1), listBo);
                postAd5 = new PostAd(account.getId(), "ВАНЕЧКА", textFive, listVan.get(2), listVan.get(1), listVan);
                postAd6 = new PostAd(account.getId(), "Энергичный пес, нуждающийся в любви и внимании!", textSix, listZu.get(0), listZu.get(2), listZu);
